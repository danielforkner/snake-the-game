...
function toggleDialogue() {
  if (
    dialogueArea.classList.contains('down') ||
    dialogueArea.classList.contains('goDown')
  ) {
...
function updateGameArea(newHead, oldHead, lostTail) {
  ...
  if (lostTail.length === 2) {
    ...
    if (remove !== undefined) {
      // undefined when game starts
      if (
        remove.classList.contains('head') ||
        remove.classList.contains('leftHead')
      ) {
